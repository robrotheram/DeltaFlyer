...
class Tile:
    ...
    def getchunk(self,z):
        ...
        img_hash = hash(tuple(self.chunk_data.pop()["map"]))
        ...
        if(img_hash in self.rendered_chunks):
          ...
          #  print("rendering new chunk")
            for i in range(0,(width*width),1):
                ...
                im = Image.open(self.getimg(i))
                im = im.resize((imgsize, imgsize), Image.BILINEAR)
                new_im.paste(im, ((x*imgsize),(y*imgsize)))

            self.rendered_chunks[img_hash]=new_im
            return new_im

    def get4chunks(self,z):
        z = int(math.pow(2,(self.max_zoom -(z))))
        width = z
        imgsize = math.floor((500/z))
        new_width = imgsize*z #

        new_im = Image.new('RGB', (new_width,new_width))#
        for i in range(0,(width*width),1):
            x = i % width
            y = math.floor(i / width)
            im = self.getchunk(z)
        ...
        new_im = new_im.resize((500, 500), Image.BILINEAR)#resize up to 512 to remove border

        new_size = (512 , 512)
        old_size = new_im.size
        border = Image.new("RGB", new_size)   ## luckily, this is already black!
        border.paste(new_im, (6,6))
        return border
